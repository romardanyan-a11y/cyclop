...
final class NotchScreenPanel {
    ...
    private func presentCentered() {
        ...
        // Явно, а не только через флаг: ключ берётся в его didSet, а тот
        // молчит, если флаг уже стоял с прошлого раза. Окно могло за это время
        // ключ потерять — например, отдав его чужому окну, — и тогда стрелки
        // не пришли бы никуда.
        panel?.makeKey()
